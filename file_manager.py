...
def display_welcome():
    ...
    print() # the code is now fixed by adding a blank line 
...
def calculate_file_size():
    ...
    try:
        ...
        size_kb = size_bytes / 1024 # The code is now fixed by changing // to /
        size_mb = size_kb / 1024    # The code is by changing // to / which changes the formula the correct
...
def process_user_command(choice, running,*,# it is fixed because it adds * for keyword-only args
                         show_goodbye= True,# it is fixed because now its adds default value
                         goodbye_message= "Thank you for using Python CLI File Manager!",# it is fixed 
                        invalid_choice_prefix="Invalid choice:",# fixed
                         valid_commands="help, calc, info, quit"):# fixed 
    """
